...
{
    public class CashBookControler
    {
        ...
        internal DataTable getCashbookDetails(string fromDate, string toDate)
        {
            conString = Program.ConnectionString.ToString();
            con = new MySqlConnection(conString);
            string sql = "";
            ...
            try
            {
                if (con.State.ToString() != "Open")
                {
                    ...
                    if (con != null)
                    {
                        sql = "SELECT tpcb.*, tpu.UserName FROM tbl_pos_cash_book tpcb INNER JOIN tbl_pos_user tpu ON tpcb.cUser = tpu.ID WHERE tpcb.cDate BETWEEN '" + fromDate + "' AND '" + toDate + "';";
