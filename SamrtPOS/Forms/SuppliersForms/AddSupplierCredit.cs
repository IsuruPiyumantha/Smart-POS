...
{
    public partial class AddSupplierCredit : Form
    {
        ...
        public AddSupplierCredit()
        {
            InitializeComponent();
            SetColors();
        }

        public AddSupplierCredit(string p,string Supplier)
        {
            this.id = p;
            ...
            if (!Error)
            {
                ...
                try
                {
                    if (con.State.ToString() != "Open")
                    {
                        ...
                        if (con != null)
                        {
                            sql = "INSERT INTO tbl_pos_suppliers_info (SupID, Description, Debit, Credit, cDate, cUser) VALUES ('" + this.id + "', @Des, 0, @Amt, '" + nowDate + "', '" + cUser + "'); INSERT INTO tbl_pos_cash_book (Description, Credit, Debit, cUser, cDate) VALUES (@description, @Amt, 0, '" + cUser + "', '" + nowDate + "');";
                            MySqlCommand cmd = new MySqlCommand(sql, con);
                            string description = "Got a money loan from " + this.SupplierName;
