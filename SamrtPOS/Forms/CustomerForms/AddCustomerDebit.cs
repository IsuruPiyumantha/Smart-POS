...
{
    public partial class AddCustomerDebit : Form
    {
        ...
        public AddCustomerDebit()
        {
            InitializeComponent();
            SetColors();
        }

        public AddCustomerDebit(string p,string CustName)
        {
            this.id = p;
            ...
            if(!Error)
            {
                ...
                try
                {
                    if (con.State.ToString() != "Open")
                    {
                        ...
                        if (con != null)
                        {
                            sql = "INSERT INTO tbl_pos_customers_info (CusID, Description, Debit, Credit, cDate, cUser) VALUES ('" + this.id + "', @Des, @Amt, 0, '" + nowDate + "', '" + cUser + "'); INSERT INTO tbl_pos_cash_book (Description, Credit, Debit, cUser, cDate) VALUES (@description, @Amt, 0, '" + cUser + "', '" + nowDate + "');";
                            MySqlCommand cmd = new MySqlCommand(sql, con);
                            string description = this.CustomerName + " received money from the debtor";
