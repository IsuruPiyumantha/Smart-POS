...
namespace SmartPOS.Forms.ItemsForms
{
    public class ItemsCategoryControls
    {
        ...
        internal bool DeleteItemsSubCategory(string x)
        {
            bool IsTrue = false;
            ...
            try
            {
                if (con.State.ToString() != "Open")
                {
                    ...
                    if (con != null)
                    {
                        sql = "DELETE FROM tbl_pos_item_sub_category WHERE ID = @id;";
                        MySqlCommand cmd = new MySqlCommand(sql, con);
                        cmd.Parameters.Add("@id", x);
                        cmd.ExecuteNonQuery();
                        IsTrue = true;
        ...
        internal CategoryInfo SelectSubCategoryInfo(string id)
        {
            CategoryInfo catInfo = new CategoryInfo();
            ...
            try
            {
                if (con.State.ToString() != "Open")
                {
                    ...
                    if (con != null)
                    {
                        sql = "SELECT * FROM tbl_pos_item_sub_category WHERE ID = '" + id + "';";
                        MySqlDataAdapter da = new MySqlDataAdapter(sql, con);
                        DataSet ds = new DataSet();
                        da.Fill(ds, "catInfo");
                        dt1 = ds.Tables["catInfo"];
                        ...
                        if (dt1.Rows.Count > 0)
                        {
                            ...
                            {
                                catInfo.ID = int.Parse(r["ID"].ToString());
                                catInfo.CategoryName = r["SubCategoryName"].ToString();
                            }
                        }
        ...
        internal bool AddNewSubCategory(string name, string p)
        {
            bool IsTrue = false;
            ...
            try
            {
                if (con.State.ToString() != "Open")
                {
                    ...
                    if (con != null)
                    {
                        sql = "INSERT INTO tbl_pos_item_sub_category (CategoryID, SubCategoryName) VALUES ('" + p + "', @name);";
                        MySqlCommand cmd = new MySqlCommand(sql, con);
                        cmd.Parameters.Add("@name", name);
                        cmd.ExecuteNonQuery();
                        IsTrue = true;
