...
{
    public partial class EditRestaurantProduct : Form
    {
        public static ProductControler prodCont;
        public static RestaurantProductInfo restProdInfo;
        public static CommonFuntion CmnFun;
        private string id;

        public EditRestaurantProduct()
        {
            InitializeComponent();
        }

        public EditRestaurantProduct(string id)
        {
        ...
        private void GetTableInfo(string id)
        {
            ...
            restProdInfo = new RestaurantProductInfo();

            restProdInfo = prodCont.GetRestaurantProductInfo(id);

            if (restProdInfo != null)
            {
                txtItemCode.Text = restProdInfo.ItemCode.ToString();
                txtItemName.Text = restProdInfo.ItemName.ToString();
                combUnit.SelectedValue = restProdInfo.Unit;
                txtLabledPrice.Text = restProdInfo.LabledPrice.ToString();
                txtBuyingCost.Text = restProdInfo.BuyingCost.ToString();
                combCategory.SelectedValue = restProdInfo.Category;
                txtSupplier.SelectedValue = restProdInfo.Supplier;
                txtBarCode.Text = restProdInfo.BarCode.ToString();

                if (restProdInfo.KOT == true)
                {
                    chkBoxKOT.Checked = true;
                }
                else
                {
            ...
            if (string.IsNullOrEmpty(txtItemName.Text.ToString()) == false || string.IsNullOrWhiteSpace(txtItemName.Text.ToString()) == false)
            {
                if (string.IsNullOrEmpty(txtLabledPrice.Text.ToString()) == false || string.IsNullOrWhiteSpace(txtLabledPrice.Text.ToString()) == false)
                {
                    ...
                    else
                    {
                        ...
                        if (prodCont.UpdateRestaurantProduct(prodInfo))
                        {
