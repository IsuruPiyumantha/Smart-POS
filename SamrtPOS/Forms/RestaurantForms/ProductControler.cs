...
namespace SmartPOS.Forms.RestaurantForms
{
    public class ProductControler : BaseControler
    {
        ...
        internal bool AddNewRestaurantProduct(RestaurantProductInfo prodInfo)
        {
            bool IsTrue = false;
            ...
            try
            {
                if (con.State.ToString() != "Open")
                {
                    ...
                    if (con != null)
                    {
                        sql = "INSERT INTO tbl_pos_items (ID, item_name, barcode, unit, labled_price, buying_cost, Category, supplier, kot_item) VALUES (@itemCode,@itemName,@barcode,@unit, @labledPrice, @buyingCost, @category, @supplier, @kot);";
                        ...
                        cmd.Parameters.Add("@kot", Convert.ToInt16(prodInfo.KOT));
                        cmd.ExecuteNonQuery();
                        IsTrue = true;
                    }
                }
        ...
        internal RestaurantProductInfo GetRestaurantProductInfo(string id)
        {
            RestaurantProductInfo getInfo = new RestaurantProductInfo();
                    ...
                    if (con != null)
                    {
                        ...
                        if (dt1.Rows.Count > 0)
                        {
                            foreach (DataRow r in dt1.Rows)
                            {
                                ...
                                getInfo.Supplier = int.Parse(r["supplier"].ToString());
                                int kot = int.Parse(r["kot_item"].ToString());
                                getInfo.KOT = Convert.ToBoolean(kot);
        ...
        internal bool UpdateRestaurantProduct(RestaurantProductInfo prodInfo)
        {
            bool IsTrue = false;
